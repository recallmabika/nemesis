...
(function () {
  ...
  form.addEventListener('submit', function (e) {
    ...
    if (!name) {
      if (fieldName) fieldName.classList.add('has-error');
      hasError = true;
    }

    if (!email || !emailRegex.test(email)) {
      if (fieldEmail) fieldEmail.classList.add('has-error');
      hasError = true;
    }

    if (!details || details.length < 5) {
      if (fieldDetails) fieldDetails.classList.add('has-error');
      hasError = true;
    }
    ...
    fetch('/api/inquiry', {
      ...
      .then(function (result) {
        if (result.ok && result.data.success) {
          showFeedback(result.data.message || 'Inquiry received. Thank you — our team will be in touch shortly.', true);
          form.reset();
          // Reset custom select to first option
          var customSelect = document.getElementById('categorySelect');
          if (customSelect) {
            var firstOpt = customSelect.querySelector('.select-option');
            var valSpan = customSelect.querySelector('.select-val');
            var opts = customSelect.querySelectorAll('.select-option');
            ...
            if (firstOpt) {
              ...
              if (categoryInput) categoryInput.value = firstOpt.getAttribute('data-value');
            }
          }
        } else {
          showFeedback(result.data.error || 'Unable to send inquiry. Please try again or email info@nemesis.co.zw directly.', false);
        }
      })
      .catch(function (err) {
        showFeedback('A network error occurred. Please check your connection or email info@nemesis.co.zw directly.', false);
      })
      .finally(function () {
        if (submitBtn) {
